...
@IF NOT EXIST assets\origshell.txt For /f delims^=^ eol^= %%a in ('CScript //nologo "modules\getshell.vbs"') do @(
set origshell="%%a"
echo "%%a">assets\origshell.txt
)
@IF NOT defined origshell For /f delims^=^ eol^= %%a in ('type assets\origshell.txt') do @set origshell=%%a
